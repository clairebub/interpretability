...
class ImageClustering(object):
    ...
    def __init__(self, folder_path="data", n_clusters=10, max_examples=None, use_imagenets=None, use_pca=False):
    ...
    def get_new_imagevectors(self):
        if self.use_imagenets is None:
            ...
            if self.use_imagenets.lower() == "vgg16":
                ...
                model1 = keras.applications.mobilenet_v2.MobileNetV2(input_shape=(224, 224, 3), alpha=1.0, depth_multiplier=1, include_top=False, weights='imagenet', pooling=None)
            else:
                raise RuntimeError(
...
if __name__ == "__main__":
    ...
    if use_imagenets is False:
        use_pca = False
    else:
        use_pca = False  # Make it True if you want to use PCA for dimensionality reduction -> Default is: False

    temp = ImageClustering(data_path, number_of_clusters, max_examples, use_imagenets, use_pca)
    temp.load_images()
