...
task_options = ['skin']
model_options = ['unet']
model_type_options = ['base', 'ensemble']
...
parser.add_argument('--dataset', default='isic2018', choices=dataset_options)
parser.add_argument('--model', default='unet', choices=model_options)
...
folder_name = "segmentation/" + args.model_type + '_' + args.task
...
if args.train:
    ...
    if args.gradcam:
        ...
        valid_data = classifier_dataloader.ImageFolderWithPaths(root='data/%s/%s_segmentation_validation%s' % (args.dataset, args.task, '' if args.validation else '_full'), transform=valid_transform)
