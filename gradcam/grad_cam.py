...
class GradCAM(_BaseWrapper):
    ...
    def generate_base(self, target_layer):
        ...
        # ToDo: concatenate list generated by multiple GPUs (order needs to be checked)
    ...
    def generate_ensemble(self, target_layers):
        ...
        for target_layer in target_layers:
            # fmaps = torch.cat(self._find(self.fmap_pool, target_layer), dim=0).cpu()

            fmaps.append(torch.cat(self._find(self.fmap_pool, target_layer), dim=0).cpu())
            grads = torch.cat(self._find(self.grad_pool, target_layer), dim=0)
            weights.append(self._compute_grad_weights(grads).cpu())
        ...
        gcam = torch.mul(fmaps[0], weights[0]).sum(dim=1, keepdim=True) + torch.mul(fmaps[1], weights[1]).sum(dim=1, keepdim=True)
